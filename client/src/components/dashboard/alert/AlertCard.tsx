    ...
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 hover:border-cyan-500/30 transition-all duration-300">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
        ...
        <div className="flex gap-2">
          <Badge className={`${severityColors[alert.severity]} border uppercase text-xs font-semibold`}>
            {alert.severity}
          </Badge>
          <Badge className={`${statusColors[alert.status]} border uppercase text-xs font-semibold`}>
            {alert.status}
          </Badge>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-800">
        <div className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <span>{format(new Date(alert.detected_at || alert.created_date), "MMM d, yyyy HH:mm")}</span>
        </div>
        {alert.source_ip && (
          <div className="flex items-center gap-2 text-sm text-gray-400">
